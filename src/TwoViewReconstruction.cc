...
/**
 * @brief 使用带有标签信息的两视图重建3D点云。
 * 
 * 该函数接收两帧的关键点、匹配关系和相对位姿，尝试通过三角化恢复3D点，并筛选出有效的3D点。
 * 
 * @param vKeys1 第一帧的关键点集。
 * @param vKeys2 第二帧的关键点集。
 * @param vMatches12 从第一帧到第二帧的匹配关系列表，每个元素表示vKeys1中对应关键点在vKeys2中的匹配索引。
 * @param T21 第二帧相对于第一帧的位姿变换，使用Sophus::SE3f表示。
 * @param vP3D 输出参数，用于存储重建后的3D点云坐标。
 * @param vbTriangulated 输出参数，用于存储每个匹配是否成功三角化的标志位向量。
 *
 * @return 如果重建成功且至少有50个有效三维点，则返回true；否则返回false。同时检查视差是否足够大以确保重建质量。
 */
