...
/**
 * @brief 通过Bag of Words (BoW)模型在关键帧pKF和当前帧F之间进行特征匹配。
 *
 * 此函数使用BoW模型来加速关键点之间的匹配过程，仅在属于相同词汇节点的ORB特征之间进行匹配。
 * 它还检查地图点的状态，确保它们没有标记为坏的，并应用最近邻比值测试以过滤出好的匹配。
 * 如果启用了方向检查，则还会根据方向直方图进一步优化匹配结果。
 *
 * @param pKF 要与之匹配的关键帧。
 * @param F 当前帧，包含要与关键帧pKF中的地图点进行匹配的特征描述符。
 * @param vpMapPointMatches 匹配结果输出，存储每个地图点对应的当前帧中的特征位置索引。初始化为NULL数组。
 *
 * @return 成功找到的对应关系数量（nmatches）。
 */
...
/**
 * @brief 通过Bag of Words (BoW)模型在两个关键帧之间进行特征匹配。
 *
 * 此函数使用BoW模型来加速关键点之间的匹配过程。它仅在属于相同词汇节点的ORB特征之间进行匹配，
 * 并检查地图点的状态，确保它们没有标记为坏的。同时应用最近邻比值测试以过滤出好的匹配。
 * 如果启用了方向检查，则还会根据方向直方图进一步优化匹配结果。
 *
 * @param pKF1 第一个关键帧，包含要与第二个关键帧pKF2中的地图点进行匹配的特征描述符和关键点。
 * @param pKF2 第二个关键帧，同样包含地图点、描述符和关键点信息。
 * @param vpMatches12 匹配结果输出，存储每个地图点在第一个关键帧中对应的第二个关键帧中的特征位置索引。初始化为NULL数组。
 *
 * @return 成功找到的对应关系数量（nmatches）。
 */
